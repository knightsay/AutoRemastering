...
# 			img = torch.from_numpy(src_dataset._load_img(src_paths[int(src_id[i])]))
# 			r0,r1,c0,c1 = src_coords[int(src_id[i])]
# 			a = img[:,r0:r1,c0:c1].unsqueeze(0)
# 			img = torch.from_numpy(dst_dataset._load_img(dst_paths[int(dst_id[int(src_id[i]), int(knn[i])])]))
